...
async def startDeliveryPackage(packageDataInput):
    try:
        ...
        newPackageData= packageDataInput.dict()
        newPackageData["deliveryStatus"] = "in_transit"
...
async def startDeliveryPackageDUMMY(packageId):
    try:
        db = get_db()
        packageCollection = db.collection("packageCollection")
        packageDoc = packageCollection.document(packageId).get()
        ...
        if not packageDoc.exists: 
            raise HTTPException(
                ...
                detail={
                    "status": "fail",
                    "message": f"Paket dengan id '{packageId}' tidak ditemukan.",
                    "timestamp": datetime.now().isoformat()
                }
            )
